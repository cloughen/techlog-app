    ...
    describe 'ログ投稿機能の検証' do
        ...
        context 'ログインしている場合' do
            ...
            it 'ログインページへリダイレクトしない' do
                expect(current_path).not_to eq('/user/sign_in')
            end
            ...
            context 'パラメータが正常な場合' do
                it 'Postを作成できる' do
                    expect do
                        subject
                        expect(page).to have_current_path(root_path, wait: 5) # 遷移完了を待つ
    ...
    describe 'ログ削除機能の検証' do
        context '投稿したユーザーでログインしている場合' do
            ...
            it '削除ボタンをクリックすると削除できる' do
                ...
                expect(page).not_to have_link("/posts/#{@post.id}") # 削除した投稿(の詳細ページへにリンク)が存在しない
            end
        end
